 ...
 *   per BUILD-PHASE-3A-REVISED-PLAN.md §3a-pre.2.
 *
...
/**
 * Seeds an adoption's Solution Intelligence Graph (SIG) root node.
 *
 ...
 *   SI_BACKEND          'neo4j' | 'polygraph'   (default '@adopt:default-backend')
...
// ─── Adoption profile (the adopter answers from BUILD-ARCHETYPES-SI-ADOPTION-PLAN.md) ──
...
const IDENTITY_HTTP_PORT = 3101; // @adopt:identity-http-port
...
// @adopt:default-namespace  Default namespace when SI_NAMESPACE/ASI_NAMESPACE are absent.
const NAMESPACE =
  process.env.SI_NAMESPACE ?? process.env.ASI_NAMESPACE ?? '@adopt:default-namespace';
// @adopt:default-backend  'neo4j' or 'polygraph'. Adopters with embedded PolyGraph default to 'polygraph'.
const BACKEND_KIND = ((): 'neo4j' | 'polygraph' => {
  ...
  return '@adopt:default-backend' as 'neo4j' | 'polygraph';
})();
// @adopt:default-polygraph-path  PolyGraph leveldb directory (e.g. 'data/polygraph').
const POLYGRAPH_PATH = process.env.SI_POLYGRAPH_PATH ?? '@adopt:default-polygraph-path';
// @adopt:default-graph-url  Bolt URL when env override is absent. Port literal is also @adopt:default-bolt-port.
const GRAPH_URL =
  process.env.SI_GRAPH_URL ?? process.env.ASI_GRAPH_URL ?? '@adopt:default-graph-url';
const GRAPH_USER =
...
const GRAPH_PASS =
  process.env.SI_GRAPH_PASS ?? process.env.ASI_GRAPH_PASS ?? '@adopt:default-graph-pass';
...
async function seedPolyGraph(): Promise<void> {
  if (!POLYGRAPH_PATH || POLYGRAPH_PATH === '@adopt:default-polygraph-path') {
    throw new Error(
      'SI_BACKEND=polygraph requires SI_POLYGRAPH_PATH (a leveldb directory)',
    );
  }
  // // Why dynamic import: polygraph-db is not necessarily a top-level
  // // dep of every adopter; under solution-intel it lives transitively
  // // under contract-loader/agents subpackages. The dynamic import keeps
  // // the neo4j-default path importable on a node_modules that has
  // // not hoisted polygraph-db.
...
async function main(): Promise<void> {
  ...
  const baseOptions: BackendOptions = {
    ...
    polygraphPath: POLYGRAPH_PATH === '@adopt:default-polygraph-path' ? undefined : POLYGRAPH_PATH,
  };
  if (BACKEND_KIND === 'polygraph') {
    console.log(
      `[seed] namespace=${NAMESPACE} backend=polygraph path=${POLYGRAPH_PATH}`,
    );
  } else {
    console.log(
