...
**As of 2026-05-22, the reference implementation lives in this directory and is current through Phase 2.5.**
...
- `./agents/` — read-only SIG-walking agents (snapshot of `wfredricks/archetypes-solution-intelligence/agents/` @ `0.2.0-pre`, **backend-pluggable as of Phase 2.5**, refreshed 2026-05-22). Default backend in the canonical archetype is **PolyGraph** (matching contract-loader's Phase 1e direction); adopters running an existing Neo4j cluster flip the `@adopt:default-backend` marker in `agents/src/backends/types.ts` to `'neo4j'`. Public function signatures of `runCompletenessAgent` / `runBookendAuditAgent` are unchanged from Phase 1b.
...
Each subdirectory carries `// @adopt:` markers at every customizable identity-and-deployment value and at every composition site (identity, audit-ledger, eventing, graph, **storage backend**). Run `grep -rn "@adopt:" .` inside any subdirectory to see them all. As of Phase 2.5 the marker count across `reference-impl/agents/` grows by 1 (`@adopt:default-backend` in `agents/src/backends/types.ts`); Phase 1e added the same marker to `reference-impl/contract-loader/`.

Source commit for the Phase 2.5 agents refresh: `cc6bc8687ee61e17994a65e2f7be92d065155a8f` in `wfredricks/archetypes-solution-intelligence` (`main`, post PR #7).

Source commit for the Phase 1e contract-loader refresh: `3dbc71a2110a3b6a7d5f5422488102c2a4daf4e8` (post PR #6).

Source commit for the prior Phase 1c lifts (identity, cli, graph-client, scripts): `195096307965d7ccd1a5ddac5da1b09db6b77b60`.

Tag of record for the Phase 2.5 in-tree lift: `solution-intel-reference-impl-2026-05-22d`. The prior Phase 1e lift was `solution-intel-reference-impl-2026-05-22c`.
