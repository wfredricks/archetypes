...
// @adopt:agent-name-completeness
// Q: What human-readable name should the CompletenessAgent emit in its
//    `agentName` field? Shows up in every Finding the agent emits and
//    on the `agentName` row of the report envelope. Operators see this
//    when scanning report markdown.
// Default: CompletenessAgent
// Format: PascalCase, no whitespace
export const AGENT_NAME = 'CompletenessAgent';
export const AGENT_VERSION = '0.1.0-pre';

// @adopt:default-namespace
// Q: What Solution namespace does this agent query by default?
//    Must match the `@adopt:namespace` answer in identity/src/index.ts
//    so the agent reads the same SIG the rest of the substrate writes to.
//    Operators override per-run via `--namespace` or `ASI_NAMESPACE`.
// Default: asi  (matches the asi adoption profile)
// Format: [a-z][a-z0-9-]{1,15}
const DEFAULT_NAMESPACE = 'asi';

// @adopt:default-graph-url
// Q: What Bolt URL does the agent connect to when no `--graph-url` or
//    `<NS>_GRAPH_URL` env var is set? Production deployments point this
//    at a managed Neo4j endpoint; local development uses bolt://localhost
//    on whatever port the project's graph-db archetype publishes.
// Default: bolt://localhost:7689  (asi adoption convention)
// Format: bolt:// or neo4j:// URL
const DEFAULT_GRAPH_URL = 'bolt://localhost:7689';

// @adopt:default-graph-user
// Q: Default Neo4j username for unauthenticated/dev runs.
//    Production callers override via `<NS>_GRAPH_USER`. The marker
//    exists so adopters can swap in their dev-instance default.
// Default: neo4j
// Format: nonempty string
const DEFAULT_GRAPH_USER = 'neo4j';

// @adopt:default-graph-pass
// Q: Default Neo4j password for unauthenticated/dev runs.
//    Production callers override via `<NS>_GRAPH_PASS`; never commit a
//    production password as the default. The asi adoption uses the
//    constellation-neo4j dev container password.
// Default: udt-pass-2026  (asi-local dev container)
// Format: nonempty string
