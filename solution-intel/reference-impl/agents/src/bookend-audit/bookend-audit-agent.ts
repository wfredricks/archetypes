...
// @adopt:agent-name-bookend-audit
// Q: Human-readable name the BookendAuditAgent emits in its `agentName`
//    field. Mirrors @adopt:agent-name-completeness in
//    completeness/completeness-agent.ts.
// Default: BookendAuditAgent
...
// Q: Solution namespace queried by default. Must match the
//    `@adopt:namespace` answer in identity/src/index.ts and the same
//    marker in completeness/completeness-agent.ts.
// Default: asi  (asi adoption profile)
const DEFAULT_NAMESPACE = 'asi';

// @adopt:default-graph-url
// Q: Default Bolt URL when no `--graph-url` or env var is set. Mirrors
//    completeness/completeness-agent.ts.
// Default: bolt://localhost:7689
const DEFAULT_GRAPH_URL = 'bolt://localhost:7689';

// @adopt:default-graph-user
// Q: Default Neo4j username for dev runs.
// Default: neo4j
const DEFAULT_GRAPH_USER = 'neo4j';

// @adopt:default-graph-pass
// Q: Default Neo4j password for dev runs. Override via env in production.
