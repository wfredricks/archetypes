 ...
 *
 *   Ownership: solution-intel canonical. Implements the `Backend`
 *   interface against `polygraph-db@0.1.4` (LevelAdapter), bridging
 *   the two simple MATCH+RETURN sites in the agents (CompletenessAgent
 *   rule 1 + BookendAuditAgent's hypothesis read) onto PolyGraph's
 *   regex bridge with `$param` substitution. The three aggregation
 *   sites (CompletenessAgent rules 2/3/4) DO NOT go through the bridge
