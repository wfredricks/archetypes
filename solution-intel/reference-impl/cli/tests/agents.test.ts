 ...
 *   from wfredricks/archetypes-solution-intelligence/cli/tests/agents.test.ts
...
/**
 * Unit tests for `si agents` CLI wiring.
 *
 ...
 * // diagnostics). We mock `@solution-intelligence/agents` so no SIG
 * // is touched.
 */
...
vi.mock('@solution-intelligence/agents', () => {
  return {
...
describe('si agents CLI', () => {
  ...
  it('completeness run exits 1 on tooling failure', async () => {
    const mod = await import('@solution-intelligence/agents');
