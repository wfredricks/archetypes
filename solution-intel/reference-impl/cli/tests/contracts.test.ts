 ...
 *   from wfredricks/archetypes-solution-intelligence/cli/tests/contracts.test.ts
...
/**
 * CLI integration test for `si contracts list` and `si contracts show`.
 *
...
import { parseBookend, commitContract } from '@solution-intelligence/contract-loader';
import {
  contractsListCommand,
  contractsShowCommand,
} from '../src/commands/contracts.js';

const GRAPH_URL = process.env.SI_GRAPH_URL ?? 'bolt://localhost:7689';
const GRAPH_USER = process.env.SI_GRAPH_USER ?? 'neo4j';
const GRAPH_PASS = process.env.SI_GRAPH_PASS ?? 'udt-pass-2026';
