 ...
 *   from wfredricks/archetypes-solution-intelligence/contract-loader/src/commit-contract.ts
 *   @ commit 195096307965d7ccd1a5ddac5da1b09db6b77b60.
 *
 *   Ownership: solution-intel canonical. Writes the ArchiMate-flavored
 *   SIG ontology committed in `archetypes/events-spine/LEFT-BOOKEND.md`
 *   to PolyGraph. Adopter copies replace `@adopt:` markers with
 *   adopter-specific values (namespace, graph URL/user/pass).
 */
...
export interface CommitOptions {
  ...
  namespace: string;
  /** Bolt URL (defaults to the package default; see DEFAULT_GRAPH_URL). */
  graphUrl?: string;
  /** Auth user (defaults to the package default; see DEFAULT_GRAPH_USER). */
  graphUser?: string;
  /** Auth password (defaults to the package default; see DEFAULT_GRAPH_PASS). */
  graphPass?: string;
  /**
...
// @adopt:default-graph-url  Replace the host/port with the adopter's PolyGraph endpoint.
const DEFAULT_GRAPH_URL = 'bolt://localhost:7689';
// @adopt:default-graph-user  Replace if the adopter's PolyGraph uses a non-default user.
...
/**
 * Canonical wire type for `Hypothesis.verifiedAt` at the contract-loader
 * boundary is ISO-8601 string. Writers coerce on the way in; readers
 * normalize on the way out.
 *
 * // Why: Phase 1b FINDINGS Rec #1 — the agents layer had to keep a
 * // defensive `normalizeIsoString` because the loader accepted Date,
 * // Neo4j DateTime, and string interchangeably. Coercing here removes
 * // the storage-type ambiguity at the single I/O boundary; the agents
 * // helper stays as belt-and-braces but should no longer matter.
 */
function coerceVerifiedAtForWrite(input: unknown): string | null {
  if (input === null || input === undefined) return null;
  if (typeof input === 'string') return input;
  if (input instanceof Date) return input.toISOString();
  // Neo4j DateTime objects expose `toString()` returning ISO-8601.
    ...
    for (const h of graph.hypotheses) {
      ...
      await session.run(
         ...
         CREATE (ct)-[:DECLARES_HYPOTHESIS]->(n)`,
        {
          ...
          text: h.text,
          status: h.status,
          // Why: coerce to canonical ISO-8601 string at the write boundary
          // so readers never see a Date or Neo4j DateTime echoed back.
          verifiedAt: coerceVerifiedAtForWrite(h.verifiedAt ?? null),
        },
      );
      nodeCount += 1;
      edgeCount += 1;
