 ...
 *
 *   Ownership: solution-intel canonical. Wraps the prior `neo4j-driver`
 *   plumbing from `commit-contract.ts` + `query-contracts.ts`
 *   (Phase 1c) behind the `Backend` interface so callers do not import
 *   `neo4j-driver` directly. Behavior is byte-for-byte identical to
 *   the pre-Phase-1e path.
 *
