 ...
 *   under BUILD-PHASE-3-PLAN.md §3.2/§3.3. Lifted to canonical
...
// @adopt:default-audit-path
// Q: Where does this adopter store the audit ledger emitted by the
//    identity service (and any future event-emitting surface)? The
//    `exportInstance` call reads from this path; `importInstance`
//    appends to it. Convention across the substrate is
//    `data/audit/audit.jsonl`.
// Default: data/audit/audit.jsonl
// Format: path relative to the adopter's working directory.
export const ADOPTER_DEFAULT_AUDIT_PATH = 'data/audit/audit.jsonl';

// @adopt:default-grants-path
// Q: Where does this adopter store the identity grants ledger? Today
//    that's the simple-auth grants file emitted by the identity
//    service. Convention is `data/audit/grants.jsonl`.
// Default: data/audit/grants.jsonl
// Format: path relative to the adopter's working directory.
export const ADOPTER_DEFAULT_GRANTS_PATH = 'data/audit/grants.jsonl';

// @adopt:default-export-dir
// Q: Where do export tarballs land by default? Today the CLI requires
//    `--output` explicitly, so this marker is a reserved hint for a
//    future ergonomic default. Convention is `data/exports/`.
// Default: data/exports/
// Format: path relative to the adopter's working directory.
export const ADOPTER_DEFAULT_EXPORT_DIR = 'data/exports/';
...
export async function exportInstance(opts: ExportOptions): Promise<ExportArtifact> {
  ...
  const createdBy = opts.createdBy ?? '@solution-intelligence/instance';
