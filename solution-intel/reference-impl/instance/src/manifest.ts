...
// @adopt:default-substrate-version
// Q: What substrate version does the export stamp when the call site
//    does not supply one explicitly? The asi adoption today is at
//    '0.2.0-pre' (after Phase 1c-1e refactors). When a future phase
//    bumps the substrate, this marker is the canonical-side default
//    that downstream adopters lift.
// Default: 0.2.0-pre
// Format: semver-like; pre-release suffixes (`-pre`) allowed.
export const ADOPTER_DEFAULT_SUBSTRATE_VERSION = '0.2.0-pre';

// @adopt:default-instance-schema-version
// Q: What instance schema version does the export stamp when the call
//    site does not supply one? Equals the substrate version at the
//    baseline (v0.2.0-pre); diverges when the first real schema
//    migration ships and the importer needs to know what to migrate
//    FROM.
// Default: 0.2.0-pre
// Format: semver-like.
export const ADOPTER_DEFAULT_INSTANCE_SCHEMA_VERSION = '0.2.0-pre';
  ...
  if (obj.exportSchemaVersion !== EXPORT_SCHEMA_VERSION) {
    throw new Error(
      `manifest validation: unsupported exportSchemaVersion '${obj.exportSchemaVersion}' ` +
        `(this @solution-intelligence/instance build recognizes '${EXPORT_SCHEMA_VERSION}')`,
    );
  }
